...
int _GetCorrectDXGIFormat(int eCurrentFormat) {
...
static void CreateRenderTarget(IDXGISwapChain* pSwapChain) {
    for (UINT i = 0; i < NUM_BACK_BUFFERS; ++i) {
        ...
        if (pBackBuffer) {
            ...
            desc.Format = static_cast<DXGI_FORMAT>(_GetCorrectDXGIFormat(sd.BufferDesc.Format));
...
namespace DX12 {
    void Hook(HWND hwnd) {
        ...
        if (!CreateDeviceD3D12(GetConsoleWindow())) {
            printf("[!] CreateDeviceD3D12() failed.\n");
            return;
        }

        printf("[+] DirectX12: g_pd3dDevice: 0x%p\n", g_pd3dDevice);
        printf("[+] DirectX12: g_dxgiFactory: 0x%p\n", g_dxgiFactory);
        printf("[+] DirectX12: g_pd3dCommandQueue: 0x%p\n", g_pd3dCommandQueue);
        printf("[+] DirectX12: g_pSwapChain: 0x%p\n", g_pSwapChain);
        ...
        if (g_pd3dDevice) {
            ...
            static lm_size_t eclStatus = LM_HookCode(reinterpret_cast<lm_address_t>(fnExecuteCommandLists), reinterpret_cast<lm_address_t>(&hkExecuteCommandLists), reinterpret_cast<lm_address_t *>(&oExecuteCommandLists));
