...
namespace Menu {
    ...
    void LoadFontsFromFolder(FontConfig& fontconfig) {
        ...
        fs::path fullFontPath = fs::path(dllPath)/fs::path(fontconfig.fontPath);

        try {
            for (const auto& entry : fs::directory_iterator(fullFontPath)) {
            ...
            std::cerr << "Path tried: " << fullFontPath << std::endl;
        }
    }
...
#pragma endregion
        ...
        loadFontConfig(dllPath + "\\sml_config.json", fontconfig);
